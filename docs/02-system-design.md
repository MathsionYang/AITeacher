...
课件记录写入 `coursewareReviews`，历史列表只显示当前单元记录。教师可在历史表格中删除当前单元课件记录，删除时同步清理对应 `pptPlans`。
...
5. 教师在审核台查看历史版本、试卷摘要、题干、分值、知识点和题型标签；参考答案与步骤核对默认折叠，需要时手动展开。
6. 生成记录写入 `generationRecords`，含题目快照，历史表格支持删除单条记录。
...
周期测验与同步出题共享规则引擎。当前 MVP 不跨单元组卷，也不混入其他模块的题目。测验题量支持 10-100 题，必须总分 100 分并覆盖当前单元全部知识点。
...
支持从历史记录载入，并导出空白卷 PDF / 含答案 PDF。历史测验记录支持删除；新测验会读取当前单元历史题干并尽量生成不同题目。

## 7. 数据层

`storage-adapter.js` schema v8 以教师端为主：
...
- 历史课件、历史出题记录、历史测验卷均以表格展示并提供删除入口。
- 同步出题页只把 AI 生成和显示参考答案作为显性主操作，导出/导入类低频操作收进“更多”菜单。
- 同步出题题目卡片默认隐藏答案解析，避免教师审核首屏被密集解析文本淹没。
- 课件图形化必须服务理解，不能只做装饰；知识点标签换行展示，避免省略号、遮挡和价格标签重叠。

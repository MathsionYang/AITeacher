...
assert.equal(storageApi.STORAGE_SCHEMA_VERSION, 6);
...
assert.ok(storageApi.SQLITE_SCHEMA.some((sql) => sql.includes("classes")));
assert.ok(storageApi.SQLITE_SCHEMA.some((sql) => sql.includes("students")));
assert.ok(storageApi.SQLITE_SCHEMA.some((sql) => sql.includes("papers")));
assert.ok(storageApi.SQLITE_SCHEMA.some((sql) => sql.includes("submissions")));
assert.ok(storageApi.SQLITE_SCHEMA.some((sql) => sql.includes("submission_answers")));
...
assert.ok(storage.sqliteMigrationPlan().collections.classes);
...
assert.ok(loginSource.includes("教师登录"));
assert.ok(loginSource.includes("学生登录"));
assert.ok(loginSource.includes("学生端暂未开放"));
assert.ok(loginSource.includes("ai-teacher-auth-session-v1"));
assert.ok(indexSource.includes("id=\"subjectSelect\""));
assert.ok(indexSource.includes("id=\"classSelect\""));
...
assert.ok(indexSource.includes("data-tab=\"settings\""));
assert.ok(indexSource.includes("data-tab=\"courseware\">导学课件</button>"));
assert.ok(indexSource.includes("data-tab=\"practice\">同步出题</button>"));
assert.ok(indexSource.includes("data-tab=\"mistakes\">错题库</button>"));
assert.ok(indexSource.includes("data-tab=\"schedule\">周期测验</button>"));
assert.ok(indexSource.includes("data-tab=\"classes\">班级管理</button>"));
assert.ok(indexSource.includes("data-tab=\"settings\">系统设置</button>"));
assert.ok(indexSource.includes("data-tab=\"classes\""));
assert.ok(indexSource.includes("id=\"classForm\""));
assert.ok(indexSource.includes("id=\"studentForm\""));
assert.ok(indexSource.includes("id=\"settings\""));
assert.ok(indexSource.includes("class=\"settings-grid\""));
...
assert.ok(appSource.includes("classesKey"));
assert.ok(appSource.includes("studentsKey"));
...
assert.ok(appSource.includes("function renderClassroom"));
assert.ok(appSource.includes("function addClassroom"));
assert.ok(appSource.includes("function addStudent"));
...
assert.ok(stylesSource.includes(".classroom-grid"));
assert.ok(stylesSource.includes(".student-list-item"));

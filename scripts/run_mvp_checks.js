...
const stylesheetHref = indexSource.match(/<link[^>]+href="([^"]+\.css)"/)?.[1] || "styles.css";
const stylesSource = fs.readFileSync(path.join(rootDir, stylesheetHref), "utf8");
...
assert.equal(indexSource.includes("id=\"teacherNameLabel\""), false);
...
assert.ok(indexSource.includes("class=\"metric-strip dashboard-metrics\""));
...
assert.ok(indexSource.includes("min=\"10\" max=\"100\" value=\"10\" step=\"1\""));
...
assert.ok(appSource.includes("function renderHistoryTable"));
assert.ok(appSource.includes("class=\"history-table\""));
assert.ok(appSource.includes("data-action=\"delete-courseware\""));
assert.ok(appSource.includes("deleteAction: \"delete-practice\""));
assert.ok(appSource.includes("deleteAction: \"delete-schedule\""));
assert.ok(appSource.includes("function deleteCoursewareHistory"));
assert.ok(appSource.includes("function deleteGenerationRecord"));
assert.ok(appSource.includes("function generateScheduledQuestions"));
assert.ok(appSource.includes("function collectExistingQuestionStemKeys"));
assert.ok(appSource.includes("function scheduledQuestionOffset"));
assert.ok(appSource.includes("avoidedStemCount"));
...
assert.ok(stylesSource.includes(".dashboard-metrics"));
assert.ok(stylesSource.includes(".history-table-wrap"));
assert.ok(stylesSource.includes(".history-table-action"));
assert.ok(stylesSource.includes("grid-template-columns: minmax(200px, 280px) minmax(160px, 200px) minmax(140px, 160px) max-content"));
assert.ok(stylesSource.includes("#saveScheduleBtn"));
assert.ok(stylesSource.includes("width: min(520px, 96%)"));
assert.ok(stylesSource.includes("overflow-wrap: anywhere"));
assert.ok(stylesSource.includes("min-height: 190px"));
assert.ok(appSource.includes("练习路径：基础 → 变式 → 应用"));
assert.equal(appSource.includes("`${text.slice(0, 8)}…`"), false);
assert.ok(stylesSource.includes("#practice .question-card-head"));
assert.ok(stylesSource.includes("#practice .question-index"));
assert.ok(stylesSource.includes("#practice .paper-meta-item"));
assert.ok(stylesSource.includes("#practice .show-answers .answer-block"));
...
assert.ok(appSource.includes("paper-meta-review"));
...
const scheduledPaper = Array.from({ length: 12 }, (_, index) => coveredQuestion(unit.points[index % unit.points.length], index + 20));
const scheduledPrepared = ruleEngine.validatePaper(scheduledPaper, unit, { limit: 12, exactQuestionCount: 12 });
assert.equal(ruleEngine.capPaperQuestionCount(101), 100);
assert.equal(ruleEngine.capPaperQuestionCount(5), 10);
assert.equal(scheduledPrepared.questions.length, 12);
assert.equal(scheduledPrepared.summary.totalScore, 100);
assert.deepEqual(scheduledPrepared.summary.missingKnowledgePoints, []);
assert.equal(scheduledPrepared.issues.some((issue) => issue.includes("题量必须")), false);

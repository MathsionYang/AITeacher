...
- 同步出题：默认空白，只按当前所选单元知识点 AI 生成/重新生成客观练习，优先选择题、填空题、计算填空题、单位换算填空题，最多 20 题，总分 100 分，支持历史练习导入导出、页面内答题判分和解析同步
...
- 规则引擎：`rule-engine.js` 统一处理题量上限、100 分归一、单元边界、客观题型过滤、开放题拒绝和答案等价判定；`storage-adapter.js` 提供 JSON 本地数据层和 SQLite 迁移 schema
...
- 规则边界：AI 题目进入试卷前会经过 `rule-engine.js` 校验，包含当前单元边界、客观题型、开放题过滤、题量最多 20 题、总分归一为 100 分和答案可判定性提示
...
常见问题：如果点击“OCR 识别”没有结果，请先看按钮下方状态提示。提示“没有连接到本机 OCR 服务”时，说明 `start_aiteacher_ocr.bat` 没有运行或端口 `8790` 未启动；提示依赖未安装时，先运行 `python -m pip install paddlepaddle paddleocr`。

...
- 课件生成：选择单元后生成 MarkdownFlow 导学课件，支持历史课件表格、历史删除、审核编辑、授课全屏、PDF/Markdown/PPTX/JSON 导出。
- 同步出题：直接复用当前单元，也可在本页修改单元；固定生成 10 道客观题，总分 100 分，覆盖本单元全部知识点。页面按教师审核台设计，默认展示历史版本表格、试卷摘要和题目卡片，参考答案默认折叠，历史记录支持删除，导出/导入收进“更多”菜单。
- 周期测验出题：按当前单元生成 10-100 题测验卷，总分 100 分，围绕本单元知识点生成不同题目，支持历史记录表格、历史删除、空白卷 PDF 和含答案 PDF。
...
- `rule-engine.js` 在模型之后做规则校验：单元边界、客观题型、同步出题固定 10 题、周期测验 10-100 题、知识点全覆盖、总分 100、开放题过滤、答案可判定。

...
- 知识点课件：默认空白，支持 AI 生成/重新生成、进度动画、情境图解、授课全屏、历史课件导入导出、审核编辑、PDF/Markdown/PPTX 导出
...
- 代码入口：`model-client.js` 负责 OpenAI-compatible `/chat/completions` 调用；`agent-orchestrator.js` 负责编排课件 Agent、出题 Agent 和 PPT 制作 Agent
- 当前能力：支持测试连接；“知识点课件”页内可 AI 生成/重新生成课件，生成时显示进度动画并在完成后展示，可导入/导出课件 JSON；PPT 制作 Agent 可把已审核课件转成结构化 PPT 方案，`pptx-exporter.js` 在本机生成可下载 `.pptx`；“同步出题”页内可 AI 生成/重新生成同步练习，可导入/导出练习 JSON，并可直接填写答案判分、生成解析、同步到拍照批改页

...
- 同步出题：直接复用当前单元，也可在本页修改单元；固定生成 10 道客观题，总分 100 分，覆盖本单元全部知识点，支持历史记录、空白卷 PDF、含答案 PDF、JSON 导入导出和页面内判分解析。
...
不需要安装业务服务器。真实模型或 OCR 才需要分别启动本地代理。

## 模型与 Agent

不配置模型也能运行，系统会使用本地模板、规则引擎和确定性判分。
...
PaddleOCR 本地代理仍保留为内部能力，用于后续图片答案识别和人工校正流程；当前教师端一级导航不展示拍照批改模块。需要单独测试 OCR 时：

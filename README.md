...
- MarkdownFlow 导学课：默认空白，支持把当前单元知识点直接输入 AI 导学 Agent，生成只读 MarkdownFlow 内容流；保留情境图解、互动追问、授课全屏、历史导入导出、审核编辑、PDF/Markdown/PPTX 导出
...
- 学生/家长：选择教材范围，查看已生成或已导入的知识点课件，可用授课全屏模式学习，完成同步练习，直接填写答案判分，查看逐题解析、错题库、成绩趋势和周期测验。
...
- 代码入口：`model-client.js` 负责 OpenAI-compatible `/chat/completions` 调用；`agent-orchestrator.js` 负责编排课件 Agent、AI 导学课件 Agent、出题 Agent 和 PPT 制作 Agent
- 当前能力：支持测试连接；“知识点课件”页已改为 MarkdownFlow 导学课，AI 生成时直接把当前单元知识点输入导学 Agent，输出“讲解 + 互动追问 + 即时检测 + 反馈”的读模式内容流，不提供听课/音频模式；生成时显示进度动画并在完成后展示，可导入/导出课件 JSON；PPT 制作 Agent 可把已审核课件转成结构化 PPT 方案，`pptx-exporter.js` 在本机生成可下载 `.pptx`；“同步出题”页内可 AI 生成/重新生成同步练习，可导入/导出练习 JSON，并可直接填写答案判分、生成解析、同步到拍照批改页
